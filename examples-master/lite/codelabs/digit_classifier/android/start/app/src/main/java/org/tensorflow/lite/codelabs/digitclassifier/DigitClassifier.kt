...
class DigitClassifier(private val context: Context) {
  ...
  private fun initializeInterpreter() {
    ...
    //indicates how many bytes of memory we should allocate to store the input for our TensorFlow Lite model.
   // float type size: indicates how many bytes our input data type will require. We use float32, so it is 4 bytes.
    //pixelsize : indicates how many color channels there are in each pixel. Our input image is a monochrome image, so we only have 1 color channel.
...
// Finish interpreter initialization
  ...
  //input image에 있는 거 인식하는 것
  private fun classify(bitmap: Bitmap): String {
    // Preprocessing: resize the input image to match the model input shape.
    //convert the input Bitmap instance to a ByteBuffer instance to feed to the model.
